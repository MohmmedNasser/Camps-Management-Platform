...
- **Display Font**: Noto Sans Arabic — loaded from Google Fonts
- **Body Font**: Noto Sans Arabic — loaded from Google Fonts
- **Code Font**: JetBrains Mono — loaded from Google Fonts

Display and heading text uses Noto Sans Arabic at bold weight with tight letter spacing (-0.02em). Body and UI text uses Noto Sans Arabic at regular and medium weights. Identifiers that are read aloud or copied by hand — national IDs, family IDs, phone numbers, currency — use JetBrains Mono so digits stay unambiguous.
...
- Do use Noto Sans Arabic for both headings and body — never mix in a second Arabic face
