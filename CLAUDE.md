...
- **Phase 4.5 (Camp Admin displaced persons):** `assets/js/pages/displaced.js`/`displaced-create.js`/`displaced-details.js`/`displaced-edit.js` now read/write real data for the Camp Admin role only, through `getCampDisplacedPersons()`/`getDisplacedPerson()`/`toFamilyMemberPayload()` (new, `assets/js/supabase/family-members.js`) plus the already-built `addFamilyMember()`/`updateFamilyMember()`/`removeFamilyMember()`. `family_members` **is** the displaced-person table — there is no separate one — and its `family_id` is `NOT NULL`, so the real create path requires an existing family (domain rule 13) and the real edit path does not expose family reassignment (no server-side trigger repairs the old family's head on a plain `UPDATE`, only on `DELETE`). Super Admin's displaced list/detail and the displaced person's own dashboard/profile are unchanged.
- **Phase 4.6 (Camp Admin aid management):** `assets/js/pages/aid.js`/`aid-details.js` gain a real Camp Admin branch (Super Admin/displaced unchanged); `aid-create.js`/`aid-edit.js` are Camp-Admin-only pages (like `family-create.html`) and are fully real, no mock branch. New: `getCampAidDistributions()`/`getAidDistribution()`/`getSiblingDistributions()`/`updateAidDistribution()`/`deleteAidDistribution()` in `assets/js/supabase/aids.js`, `getCampFamilyOptions()` in `families.js`, `listOrganizationOptions()` in `organizations.js`, and an independent `matchesAidFilters()` in `core/selectors.js`. No RPC/migration needed for edit — RLS already grants Camp Admin full CRUD on `aid_distributions` and both junction tables, so it updates the parent row then diffs the type/family junction rows (inserts before deletes, so the row is never transiently empty). Real aid creation does not send a notification (notifications stay out of scope). Real rows carry both a family's `reference_code` (display/links, matching every existing `?familyId=` caller) and its UUID (`familyDbIds`, what the create/update calls and the beneficiary multi-select actually need) — never conflate the two.
...
- **Phase 4.8 (Camp Admin documents):** `assets/js/pages/documents.js` now reads/writes real Supabase + Cloudinary data for the Camp Admin role, through a new `getCampDocuments()` in `assets/js/supabase/documents.js` and a new `matchesDocumentFilters()` in `core/selectors.js`. Fixed two real, previously-live bugs along the way: the upload person-picker sourced mock ids that never matched a real session (upload was silently broken for every real Camp Admin since Phase 4.1), and real image preview never actually fetched a blob from `documents-access`. Also fixed a previously-invisible CORS gap in the three Cloudinary Edge Functions' shared config that silently blocked every real-browser call to them since Phase 3 — invisible until this phase because Phase 3's own tests call the functions from Node, which doesn't enforce CORS. Super Admin's document list and the displaced person's own document list/upload are unchanged, still mock — later Phase 4 work.
- **Phase 4.9 (Donor organizations):** `assets/js/pages/organizations.js` now reads/writes real data for **both** of its `PAGE_ACCESS` roles (Camp Admin and Super Admin) at once — the first Phase 4.x page with no mock branch left behind, since `organizations` has no camp scoping at all and RLS grants the two roles identical full CRUD. New `listOrganizationsWithUsage()` in `assets/js/supabase/organizations.js` and `matchesOrganizationFilters()` in `core/selectors.js`. Fixed two real, previously-latent bugs in the never-before-exercised Phase 2 `createOrganization`/`updateOrganization`: an empty optional field sent as `''` 23514s against the live phone-format check (must be `null`), and `updateOrganization` filtered its patch against snake_case keys while every real caller builds camelCase payloads, silently dropping `responsiblePerson` from every update. Per-row aid-usage counts are intentionally RLS-scoped, not platform-wide — a Camp Admin sees only their own camp's usage of a donor used across multiple camps. No backend/schema/RLS change of any kind.
